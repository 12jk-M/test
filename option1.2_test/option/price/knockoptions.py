...
from config.postgres  import  table_knock_option,table_write_tmp
    ...
    def do_all(self):
        ...
        if self.begintime and self.endtime is not None:
            ...
            while (Now>=self.begintime and Now<=self.endtime):
                ...
                knockoptions_oneday(self.Lock,self.Now)
                Now = timedelta(Now,1)
...
class knockoptions_oneday(option):
    """
    敲出式期权
    """
    ...
    def getDataFromMongo(self):
        ...
        for lst in self.data:
            ...
            type = lst['type']
            temp = self.cumputeLost(Setdate,SetRate,deliverydate,currentRate,LockedRate,kncockRate,SellRate,BuyRate,self.delta,type)
            ...
            logs('compute','knockoptions','getDataFromMongo',forwarddict,infoFalg=True)
    ...
    def  cumputeLost(self,Setdate,SetRate,deliverydate,currentRate,LockedRate,kncockRate,SellRate,BuyRate,delta,type):
       if SellRate in [None,[]] or BuyRate in [None,[]]:
           return None
       else:
           return knockoption(Setdate,SetRate,deliverydate,currentRate,LockedRate,kncockRate,SellRate,BuyRate,delta,type,self.Now)
