...
class RateExchange(object):
    ...
    def getdayMax(self,Time=None):
        ...
        data = self.__mongo.select('kline',{'type':'5','code':self.code,'Time':{'$gte':Time2,'$lte':Time1}})
        if data ==[]:
            return []
        else:
           data = data[0]
...
class BankRate(object):
    ...
    def getday(self,Time=None):
        ...
        data = self.__mongo.select('KPI',{'index':self.code,'datadate':{'$lte':Time,'$gte':Time2},'ratetype':self.ratetype})
        if data !=[]:
            data = data[-1]
            try:
