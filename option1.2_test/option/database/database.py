...
class postgersql(database):
    ...
    def select(self,tablename,colname,wherestring,orderby=None):
        ...
        try:
            ...
            traceback.print_exc()
            return None
    def view(self,sql,colname=None):
           cur = self.__conn.cursor()
    ...
    def insert(self,tablname,columnname,insertlist):
        """
        插入数据
        tablename:表名称
        columnname:字段名称
        ...
        for insert in insertlist:
            try:
                cur.execute(insertSql%(tablname,columnname,str(tuple(insert))))
            except:
                traceback.print_exc()
                continue
        cur.close()
    ...
    def deltetable(self,tablename,wherestring=None):
        """
        删除表全部数据
        tablename:表名称
        wherestring:过滤条件字符串
        ...
        cur = self.__conn.cursor()
        try:
            if wherestring is None:
                cur.execute('delete from %s'%(tablename))
            else:
                cur.execute('delete from %s where %s'%(tablename,wherestring))
        except:
            traceback.print_exc()
        cur.close()
