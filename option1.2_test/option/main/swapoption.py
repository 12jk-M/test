...
def SwapOption(Setdate,SetRate,valuedate,deliverydate,currentRate,SellRate,BuyRate,LockedRate,rateway,delta,capped_exrate,trade_type,FixRate,now):
    """
    货币掉期
    ...
    value = OrdinaryForward(SellRate,BuyRate,deliverydate,LockedRate,currentRate,now)##当前时刻普通外汇的定价
    ...
    if Now>=Setdate:
        ...
        if trade_type!='3':##非货币掉期，获得固定补贴
            prob = SwapOptionLost(currentRate,Now,Setdate,SellRate_update,BuyRate_update,delta,capped_exrate)##获得固定补贴的概率及贴现到当前时刻
            Lost = (capped_exrate - LockedRate)/currentRate*np.exp(-(BuyRate_update - SellRate_update)*T)##补贴比例
        else:
            prob=0.0
            Lost = 0.0
    ...
    if trade_type=='2':
        #区间式货币掉期(利率进行互换+固定补贴)
        return (1-prob)*(checkin - checkout  + value) + prob * Lost
...
def SwapOptionLost(currentRate,Now,deliverydate,SellRate_update,BuyRate_update,delta,capped_exrate):
    ...
    t = T/1.0
    K = capped_exrate
    d1 = np.log(S/1.0/K/np.exp(-r*t))/delta/np.sqrt(t/360.0)+delta*np.sqrt(t/360.0)/2
    d2= d1- delta*np.sqrt(t/360.0)
